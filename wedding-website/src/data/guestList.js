export const guestList = [
    // --- Groom's Side ---

    // Family - Main
    {
        id: 'g_fam_1',
        side: 'groom',
        members: [
            { name: 'Mom', id: 'm_1' },
            { name: 'Baba', id: 'b_1' },
            { name: 'Mama', id: 'm_2' }, // "BaBa" from user list, maintaining capitalization
            { name: 'Sheharyar Khan', id: 'sk_1' },
            { name: 'Daniyal Khan', id: 'dk_1' },
        ],
        maxPlusOnes: 0,
    },
    {
        id: 'g_fam_2',
        side: 'groom',
        members: [{ name: 'Aftab Khan', id: 'ak_1' }],
        maxPlusOnes: 0,
    },
    // Mona Ahmed Family
    {
        id: 'g_fam_3',
        side: 'groom',
        members: [
            { name: 'Mona Ahmed', id: 'ma_1' },
            { name: 'Yusuf Ahmed', id: 'ya_1' },
            { name: 'Alisha Ahmed', id: 'aa_1' },
            { name: 'Myra Ahmed', id: 'my_1' },
            { name: 'Reja Ahmed', id: 'ra_1' },
        ],
        maxPlusOnes: 0,
    },
    // Afsheen Khala Family
    {
        id: 'g_fam_4',
        side: 'groom',
        members: [
            { name: 'Afsheen Khala', id: 'afk_1' },
            { name: 'Ikram Burney', id: 'ib_1' },
            { name: 'Eiman Burney', id: 'eb_1' },
            { name: 'Rayyan Burney', id: 'rb_1' },
            { name: 'Zain Burney', id: 'zb_1' },
            { name: 'Mashal Burney', id: 'mb_1' },
        ],
        maxPlusOnes: 0,
    },
    // Tajwer Mirza Family
    {
        id: 'g_fam_5',
        side: 'groom',
        members: [
            { name: 'Tajwer Mirza', id: 'tm_1' },
            { name: 'Aly Mirza', id: 'am_1' },
        ],
        maxPlusOnes: 0,
    },
    // Samra Fahd Family
    {
        id: 'g_fam_6',
        side: 'groom',
        members: [
            { name: 'Samra Fahd', id: 'sf_1' },
            { name: 'Fahd Javed', id: 'fj_1' },
            { name: 'Ibrahim Javed', id: 'ij_1' },
            { name: 'Hasan Javed', id: 'hj_1' },
            { name: 'Sara Javed', id: 'sj_1' },
            { name: 'Rayyan Javed', id: 'rj_2' }, // Specify unique ID
        ],
        maxPlusOnes: 0,
    },
    // Siraj Quadar Family
    {
        id: 'g_fam_7',
        side: 'groom',
        members: [
            { name: 'Siraj Quadar', id: 'sq_1' },
            { name: 'Farah Siraj', id: 'fs_1' },
        ],
        maxPlusOnes: 0,
    },
    // Mishal Quadar Khan Family
    {
        id: 'g_fam_8',
        side: 'groom',
        members: [
            { name: 'Mishal Quadar Khan', id: 'mqk_1' },
            { name: 'Ummer Khan', id: 'uk_1' }, // Note: Duplicate name risk with Bride's side Umer Khan, but distinct here.
        ],
        maxPlusOnes: 0,
    },
    // Nimrah Quadar Khan Family
    {
        id: 'g_fam_9',
        side: 'groom',
        members: [
            { name: 'Nimrah Quadar Khan', id: 'nqk_1' },
            { name: 'Summayyah', id: 'sum_1' },
            { name: 'Saad Khan', id: 'sak_1' },
        ],
        maxPlusOnes: 0,
    },
    // Meraj Quadar Family
    {
        id: 'g_fam_10',
        side: 'groom',
        members: [
            { name: 'Meraj Quadar', id: 'meq_1' },
            { name: 'Shazia Quadar', id: 'shq_1' },
            { name: 'Noor Quadar', id: 'nq_1' },
            { name: 'Fatima Quadar', id: 'fq_1' },
            { name: 'Zain Quadar', id: 'zq_1' },
        ],
        maxPlusOnes: 0,
    },
    // Mahnoor Baji
    {
        id: 'g_fam_11',
        side: 'groom',
        members: [
            { name: 'Mahnoor Baji', id: 'mbj_1' },
            { name: 'David', id: 'dav_1' },
        ],
        maxPlusOnes: 0,
    },
    // Bilal Bhai
    {
        id: 'g_fam_12',
        side: 'groom',
        members: [{ name: 'Bilal Bhai', id: 'bb_1' }],
        maxPlusOnes: 0,
    },
    // Akhtar Uncle
    {
        id: 'g_fam_13',
        side: 'groom',
        members: [{ name: 'Akhtar Uncle', id: 'au_1' }],
        maxPlusOnes: 0,
    },
    // Samia Baji Family (Kids +3)
    {
        id: 'g_fam_14',
        side: 'groom',
        members: [
            { name: 'Samia Baji', id: 'sb_1' },
            { name: 'Faisal Bhai', id: 'fb_1' },
        ],
        maxPlusOnes: 3,
    },
    // Shan Bhai Family
    {
        id: 'g_fam_15',
        side: 'groom',
        members: [
            { name: 'Shan Bhai', id: 'shb_1' },
            { name: 'Seemi Baji', id: 'seb_1' },
            { name: 'Ayah', id: 'ayah_1' },
        ],
        maxPlusOnes: 0,
    },
    // Shijju Bhai (Wife + Daughters)
    // User says: "Shujju bhai's name and then he can add upto 3 plus ones"
    {
        id: 'g_fam_16',
        side: 'groom',
        members: [{ name: 'Shujju Bhai', id: 'shuj_1' }],
        maxPlusOnes: 3,
    },

    // --- Shaheer Friends (Groom) ---
    // Couple: Arjun & Melitza
    {
        id: 'g_friend_couple_1',
        side: 'groom',
        members: [
            { name: 'Arjun', id: 'gfc_1_1' },
            { name: 'Melitza', id: 'gfc_1_2' },
        ],
        maxPlusOnes: 0,
    },
    ...[
        'Jad', 'Taranvir (T)', 'Umer Khan', 'Irfan', 'Shahmir', 'Zaeem', 'Samer',
        'Yohan', 'Arnav', 'Andrew', 'Raj', 'Abtin', 'Asim', 'Ayaan', 'Ejaz',
        'Shayan', 'Hasnain', 'Rishaab', 'Aahil'
    ].map((name, idx) => ({
        id: `g_friend_${idx}`,
        side: 'groom',
        members: [{ name, id: `gf_${idx}` }],
        maxPlusOnes: 0,
    })),


    // --- Bride's Side (Amna) ---

    // Umer Khan Family (Bride)
    {
        id: 'b_fam_1',
        side: 'bride',
        members: [
            { name: 'Umer Khan', id: 'buk_1' },
            { name: 'Kanwal Khan', id: 'kk_1' },
            { name: 'Jannah Khan', id: 'jk_1' },
            { name: 'Amaan Khan', id: 'ak_2' },
            { name: 'Aafiyah Khan', id: 'afk_2' },
            { name: 'Shireen Khan', id: 'shk_1' },
            { name: 'Saghir Khan', id: 'sk_2' },
        ],
        maxPlusOnes: 0,
    },
    // Fahd Sharif Family
    {
        id: 'b_fam_2',
        side: 'bride',
        members: [
            { name: 'Fahd Sharif', id: 'fs_2' },
            { name: 'Sehr Sharif', id: 'ss_1' },
            { name: 'Azaan Sharif', id: 'az_1' },
            { name: 'Rayyan Sharif', id: 'rs_1' },
            { name: 'Sarah Sharif', id: 'sar_1' },
        ],
        maxPlusOnes: 0,
    },
    // Usman Khan Family
    {
        id: 'b_fam_3',
        side: 'bride',
        members: [
            { name: 'Usman Khan', id: 'usk_1' },
            { name: 'Safia Khan', id: 'saf_1' },
            { name: 'Eesa Khan', id: 'ees_1' },
            { name: 'Eman Khan', id: 'emk_1' },
        ],
        maxPlusOnes: 0,
    },
    // Dina Ali Family
    {
        id: 'b_fam_4',
        side: 'bride',
        members: [
            { name: 'Dina Ali', id: 'da_1' },
            { name: 'Faiyaz Ali', id: 'fa_1' },
            { name: 'Feroze Ali', id: 'fe_1' },
            { name: 'Zain Ali', id: 'za_1' },
        ],
        maxPlusOnes: 0,
    },
    // Sohail Rashid Family
    {
        id: 'b_fam_5',
        side: 'bride',
        members: [
            { name: 'Sohail Rashid', id: 'sr_1' },
            { name: 'Thalia Rashid', id: 'tr_1' },
            { name: 'Shaan Rashid', id: 'shr_1' },
            { name: 'Samed Asmer', id: 'sa_1' },
        ],
        maxPlusOnes: 0,
    },
    // Waseem Siddiqi Family
    {
        id: 'b_fam_6',
        side: 'bride',
        members: [
            { name: 'Waseem Siddiqi', id: 'ws_1' },
            { name: 'Amber Siddiqi', id: 'ams_1' },
            { name: 'Ammar Siddiqi', id: 'as_1' },
            { name: 'Maheen', id: 'mah_1' },
            { name: 'Asad', id: 'asa_1' }, // "Asad + kids" - leaving as Asad for now, usually implies kids come with.
            { name: 'Ahmer Siddiqi', id: 'ahs_1' },
            { name: 'Eisha Siddiqi', id: 'es_1' },
        ],
        maxPlusOnes: 3,
    },
    // Yasmin Wahid Family
    {
        id: 'b_fam_7',
        side: 'bride',
        members: [
            { name: 'Yasmin Wahid', id: 'yw_1' },
            { name: 'Hanna Wahid', id: 'hw_1' },
            { name: 'Ameera Wahid', id: 'aw_1' },
            { name: 'Hoor Wahid', id: 'how_1' },
            { name: 'Hassan Wahid', id: 'haw_1' },
        ],
        maxPlusOnes: 0,
    },
    // Hajra Khan Family
    {
        id: 'b_fam_8',
        side: 'bride',
        members: [
            { name: 'Hajra Khan', id: 'hk_1' },
            { name: 'Simra', id: 'sim_1' },
        ],
        maxPlusOnes: 0,
    },
    // Salman Ahmed Family
    {
        id: 'b_fam_9',
        side: 'bride',
        members: [
            { name: 'Salman Ahmed', id: 'sah_1' },
            { name: 'Liz Ahmed', id: 'la_1' },
            { name: 'Mayesha Ahmed', id: 'may_1' },
            { name: 'Farah Ahmed', id: 'fha_1' },
            { name: 'Natasha Ahmed', id: 'na_1' },
        ],
        maxPlusOnes: 0,
    },
    // Parveen Ahmed Family
    {
        id: 'b_fam_10',
        side: 'bride',
        members: [
            { name: 'Parveen Ahmed', id: 'pa_1' },
            { name: 'Farhan Ahmed', id: 'faha_1' },
        ],
        maxPlusOnes: 0,
    },
    // Wajahat Shariff Family
    {
        id: 'b_fam_11',
        side: 'bride',
        members: [
            { name: 'Wajahat Shariff', id: 'wsh_1' },
            { name: 'Samia Shariff', id: 'ssh_1' },
            { name: 'Ayaan Shariff', id: 'ays_1' },
            { name: 'Alia Shariff', id: 'als_1' },
        ],
        maxPlusOnes: 0,
    },
    // Faiz Iqbal Family
    {
        id: 'b_fam_12',
        side: 'bride',
        members: [
            { name: 'Faiz Iqbal', id: 'fi_1' },
            { name: 'Hala Iqbal', id: 'hi_1' },
            { name: 'Asfand Iqbal', id: 'asi_1' },
            { name: 'Lubna Iqbal', id: 'li_1' },
        ],
        maxPlusOnes: 0,
    },
    // Zaman Ahmed Family
    {
        id: 'b_fam_13',
        side: 'bride',
        members: [
            { name: 'Zaman Ahmed', id: 'zaa_1' },
            { name: 'Shazia Ahmed', id: 'sha_1' },
            { name: 'Noor Ahmed', id: 'noa_1' },
            { name: 'Hamza Ahmed', id: 'haa_1' },
            { name: 'Mohammad Ahmed', id: 'moa_1' },
        ],
        maxPlusOnes: 0,
    },
    // Zareen Naeem Family
    {
        id: 'b_fam_14',
        side: 'bride',
        members: [
            { name: 'Zareen Naeem', id: 'zan_1' },
            { name: 'Zahid Naeem', id: 'zahn_1' },
            { name: 'Sana', id: 'san_1' },
            { name: 'Ali Naeem', id: 'aln_1' },
            { name: 'Maryam Naeem', id: 'man_1' },
        ],
        maxPlusOnes: 0,
    },
    // Jamil Rashid Family
    {
        id: 'b_fam_15',
        side: 'bride',
        members: [
            { name: 'Jamil Rashid', id: 'jr_1' },
            { name: 'Fiza Rashid', id: 'fr_1' },
            { name: 'Usman Altafullah', id: 'ua_1' },
            { name: 'Ayesha Rashid', id: 'ayr_1' },
            { name: 'Soghra Ali', id: 'soa_1' },
            { name: 'Nazhat Rashid', id: 'nr_1' },
        ],
        maxPlusOnes: 0,
    },
    // Rahmat Ullah Quadri Family
    {
        id: 'b_fam_16',
        side: 'bride',
        members: [
            { name: 'Rahmat Ullah Quadri', id: 'ruq_1' },
            { name: 'Mah parveen Quadri', id: 'mpq_1' },
        ],
        maxPlusOnes: 0,
    },
    // Farheen Qazi Family
    {
        id: 'b_fam_17',
        side: 'bride',
        members: [
            { name: 'Farheen Qazi', id: 'fq_2' },
            { name: 'Naveed Qazi', id: 'nq_2' },
            { name: 'Zain Qazi', id: 'zq_2' },
            { name: 'Zubair Qazi', id: 'zuq_1' },
        ],
        maxPlusOnes: 0,
    },
    // Maryam Usman Family
    {
        id: 'b_fam_18',
        side: 'bride',
        members: [
            { name: 'Maryam Usman', id: 'mu_1' },
            { name: 'Ayaz Usman', id: 'au_2' },
            { name: 'Alia Usman', id: 'alu_1' },
            { name: 'Yousuf Usman', id: 'yu_1' },
            { name: 'Haroon Usman', id: 'hu_1' },
        ],
        maxPlusOnes: 0,
    },
    // Hamza Quadri Family
    {
        id: 'b_fam_19',
        side: 'bride',
        members: [
            { name: 'Hamza Quadri', id: 'hq_1' },
            { name: 'Mahrukh Quadri', id: 'mq_1' },
            { name: 'Mustafa Quadri', id: 'muq_1' },
        ],
        maxPlusOnes: 0,
    },
    // Sarfaraz Ahmed Family
    {
        id: 'b_fam_20',
        side: 'bride',
        members: [
            { name: 'Sarfaraz Ahmed', id: 'sa_2' },
            { name: 'Sohail Ahmed', id: 'soa_2' },
            { name: 'Ayesha Ahmed', id: 'aya_1' },
            { name: 'Maimoona Ahmed', id: 'ma_2' },
            { name: 'Zaki Ahmed', id: 'za_2' },
        ],
        maxPlusOnes: 0,
    },
    // Kaiser Khwaja Family
    {
        id: 'b_fam_21',
        side: 'bride',
        members: [
            { name: 'Kaiser Khwaja', id: 'kk_2' },
            { name: 'Sadia Khwaja', id: 'sk_3' },
            { name: 'Owais Khwaja', id: 'ok_1' },
            { name: 'Meher Khwaja', id: 'mek_1' },
            { name: 'Hassan Khwaja', id: 'hk_2' },
            { name: 'Asra Khwaja', id: 'ask_1' },
            { name: 'Shoaib Khwaja', id: 'shk_2' },
        ],
        maxPlusOnes: 0,
    },
    // Ayesha Qazi Family
    {
        id: 'b_fam_22',
        side: 'bride',
        members: [
            { name: 'Ayesha Qazi', id: 'aq_1' },
            { name: 'Adil Qazi', id: 'adq_1' },
            { name: 'Saad Qazi', id: 'sq_2' },
            { name: 'Asad Qazi', id: 'asq_1' },
            { name: 'Rafay Qazi', id: 'rq_1' },
        ],
        maxPlusOnes: 0,
    },
    // Ozma Rahman Family
    {
        id: 'b_fam_23',
        side: 'bride',
        members: [
            { name: 'Ozma Rahman', id: 'or_1' },
            { name: 'Aziz Rahman', id: 'ar_1' },
            { name: 'Maazin Rahman', id: 'mr_1' },
            { name: 'Afroze Rahman', id: 'afr_1' },
            { name: 'Hassan Rahman', id: 'hr_1' },
        ],
        maxPlusOnes: 0,
    },
    // Najma Quadri
    {
        id: 'b_fam_24',
        side: 'bride',
        members: [{ name: 'Najma Quadri', id: 'nq_3' }],
        maxPlusOnes: 0,
    },
    // Faisal Quadri Family
    {
        id: 'b_fam_25',
        side: 'bride',
        members: [
            { name: 'Faisal Quadri', id: 'fq_3' },
            { name: 'Henna Quadri', id: 'heq_1' },
            { name: 'Fyza Quadri', id: 'fyq_1' },
            { name: 'Zaid Quadri', id: 'zq_3' },
        ],
        maxPlusOnes: 0,
    },
    // Haniya Khan Family
    {
        id: 'b_fam_26',
        side: 'bride',
        members: [
            { name: 'Haniya Khan', id: 'hak_1' },
            { name: 'Naveed Khan', id: 'nk_1' },
            { name: 'Samiya Khan', id: 'sak_2' },
            { name: 'Usman Khan', id: 'uk_2' },
        ],
        maxPlusOnes: 0,
    },
    // Shameem Quadri Family
    {
        id: 'b_fam_27',
        side: 'bride',
        members: [
            { name: 'Shameem Quadri', id: 'shq_2' },
            { name: 'Talat Quadri', id: 'tq_1' },
            { name: 'Sanya Quadri', id: 'sq_3' },
            { name: 'Hera Quadri', id: 'heq_2' },
            { name: 'Alfaz', id: 'al_1' },
            { name: 'Hareem Quadri', id: 'har_1' },
        ],
        maxPlusOnes: 0,
    },
    // Faheem Quadri Family
    {
        id: 'b_fam_28',
        side: 'bride',
        members: [
            { name: 'Faheem Quadri', id: 'faq_1' },
            { name: 'Fahimina Quadri', id: 'fiq_1' },
            { name: 'Marij Quadri', id: 'maq_1' },
            { name: 'Soha Quadri', id: 'soq_1' },
            { name: 'Zuhair Quadri', id: 'zuq_2' },
        ],
        maxPlusOnes: 0,
    },
    // Maryam Quadri Family
    {
        id: 'b_fam_29',
        side: 'bride',
        members: [
            { name: 'Maryam Quadri', id: 'maq_2' },
            { name: 'Tariq Ali', id: 'ta_1' },
            { name: 'Mikail Ali', id: 'mia_1' },
        ],
        maxPlusOnes: 0,
    },
    // Zehra Quadri Family
    {
        id: 'b_fam_30',
        side: 'bride',
        members: [
            { name: 'Zehra Quadri', id: 'zq_4' },
            { name: 'Hassan Sulehri', id: 'hs_1' },
            { name: 'Anaya Sulehri', id: 'as_2' },
            { name: 'Aiza Sulehri', id: 'ai_1' },
        ],
        maxPlusOnes: 0,
    },
    // Asra Zaman Family
    {
        id: 'b_fam_31',
        side: 'bride',
        members: [
            { name: 'Asra Zaman', id: 'az_2' },
            { name: 'Rafaay Zaman', id: 'rz_1' },
            { name: 'Wasay Zaman', id: 'wz_1' },
            { name: 'Irfan Zaman', id: 'iz_1' },
        ],
        maxPlusOnes: 0,
    },
    // Asim Parekh Family
    {
        id: 'b_fam_32',
        side: 'bride',
        members: [
            { name: 'Asim Parekh', id: 'ap_1' },
            { name: 'Sadaf Parekh', id: 'sp_1' },
            { name: 'Zunaira Parekh', id: 'zp_1' },
            { name: 'Ayesha Parekh', id: 'ayp_1' },
            { name: 'Mohammed Parekh', id: 'mop_1' },
        ],
        maxPlusOnes: 0,
    },
    // Tanveer Ajmal Family
    {
        id: 'b_fam_33',
        side: 'bride',
        members: [
            { name: 'Tanveer Ajmal', id: 'taj_1' },
            { name: 'Shazia Fazal', id: 'sf_2' },
            { name: 'Aashir Ajmal', id: 'aa_2' },
            { name: 'Zara Ajmal', id: 'za_3' },
        ],
        maxPlusOnes: 0,
    },
    // Ghazala Quadri
    {
        id: 'b_fam_34',
        side: 'bride',
        members: [{ name: 'Ghazala Quadri', id: 'gq_1' }],
        maxPlusOnes: 0,
    },
    // Azmat Nasir Family
    {
        id: 'b_fam_35',
        side: 'bride',
        members: [
            { name: 'Azmat Nasir', id: 'an_1' },
            { name: 'Ayesha Azmat', id: 'aa_3' },
        ],
        maxPlusOnes: 3,
    },

    // --- Bride's Friends ---
    // Couple: Amani & Prits
    {
        id: 'b_friend_couple_1',
        side: 'bride',
        members: [
            { name: 'Amani', id: 'bfc_1_1' },
            { name: 'Prits', id: 'bfc_1_2' },
        ],
        maxPlusOnes: 0,
    },
    // Couple: Rachita & Dan
    {
        id: 'b_friend_couple_2',
        side: 'bride',
        members: [
            { name: 'Rachita', id: 'bfc_2_1' },
            { name: 'Dan', id: 'bfc_2_2' },
        ],
        maxPlusOnes: 0,
    },
    ...[
        'Shawdie', 'Bismah', 'Rytha', 'Aqsa', 'Ayesha', 'Kathir', 'Sid', 'Anushka',
        'Sam', 'Sahil', 'Rastogi', 'Nadin', 'Ghania', 'Aleena',
        'Lamees', 'Elena', 'Mahdi', 'Aryan', 'Aditi', 'Ana',
        'Muhammad anjum', 'Dana', 'Judy'
    ].map((name, idx) => ({
        id: `b_friend_${idx}`,
        side: 'bride',
        members: [{ name, id: `bf_${idx}` }],
        maxPlusOnes: 0,
    })),
];
